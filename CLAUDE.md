...
`Task.status` (`'active' | 'completed' | 'cancelled'`) is one field rather than two booleans, because done and cancelled exclude each other and separate flags would let both be true. It is not derived from `progress`: marking a task done also sets `progress` to 100, but clearing the status leaves whatever progress the user had, and cancelling never touches progress at all — the work stopped partway and that number should survive an undo. `asStatus()` in `normalizeTask` still reads the older `completed: true` flag, so records written before this field load correctly; an explicit `status` wins over it.

Status is rendered in several places that must stay consistent. In `GanttRow` the color dot doubles as the quick toggle (active → completed, completed or cancelled → active) and the row shows a pill — green tick "Bitti" or grey cross "İptal" — desktop only, since on mobile the always-visible reorder buttons leave the name no room; the pill is also dropped when the task column minus the indent falls under 260px. The chart row picks up a matching wash (green / grey) so the state reads across the whole timeline. In `GanttBar` a closed task gets a ring, a filled badge and a strikethrough name: completed keeps its color at `saturate-50` with an emerald ring, cancelled goes fully grey (`saturate-0`, neutral ring) so it cannot be mistaken for a finished one. The badge needs its own solid background — a thin ring or a bare icon disappeared on the lighter bar colors — and it is dropped below 28px of bar width. `ringClass` resolves invalid-range / dragging / completed / cancelled in that order so two `ring-*` classes never fight.

`TaskModal` holds the same two options as a radio group; pressing the selected one returns the task to `active`, which is why there is no third "devam ediyor" button.
